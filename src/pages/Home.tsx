...
import { HERO_LINE, PROCESS, ORIGINS } from "../content/site";
...
export default function Home() {
  return (
    <>
      {/* HERO */}
      <section className="relative flex min-h-[88vh] items-end overflow-hidden bg-bg-film">
        <div className="absolute inset-0"><MediaPlaceholder label="Hero loop: slow drift over misted high-grown coffee ridgelines (licensed stock, generic)" kind="film" ratio="auto" /></div>
        <div className="hero-scrim" />
        <div className="container-page relative z-10 pb-16 pt-28">
          <p className="eyebrow">Multi-origin &middot; high-grown &middot; provably real</p>
          <h1 className="display mt-4 text-fg" style={{ fontSize: "var(--step-kinetic)" }}>Proven,<br />not promised.</h1>
      ...
      {/* PROCESS TEASER (full scroll film in Phase 3) */}
      <section className="container-page py-20" aria-labelledby="process-h">
        ...
        <ol className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {PROCESS.map((p, i) => (
            <li key={p.word}>
              <MediaPlaceholder label={`${p.word}: generic coffee craft (licensed stock)`} kind="film" ratio="4 / 5" />
      ...
      <section className="bg-bg-warm py-20" aria-labelledby="auth-h">
        <div className="container-page grid gap-12 lg:grid-cols-2">
          ...
          <div className="relative">
            <MediaPlaceholder label="Beans in a jute sack, low light (licensed stock, generic)" kind="photo" ratio="4 / 3" />
      ...
      <section className="container-page py-20" aria-labelledby="drop-h">
        <div className="grid gap-12 lg:grid-cols-2">
          <div className="relative"><MediaPlaceholder label="Strawberry Hill Reserve bag (supply real product shot)" kind="product" ratio="1 / 1" /></div>
          <div>
      ...
      <section id="origins" className="bg-bg-warm py-20" aria-labelledby="origins-h">
        <div className="container-page">
          ...
          <ul className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {ORIGINS.map((o) => (
              <li key={o.name} className="flex min-h-[220px] flex-col justify-end rounded border border-line bg-bg-film p-5">
